function main()
    ...
    popsize = 200; % velkost populacie
    maxgen = 300; % max pocet gen (teda aj cyklov foru)
    mut_rate = 0.1; % pravdepodobnost mutacie 
    elite_count = 2; % kolko najlepsich jedincov prezije bez zmeny (elitarizmus)
    Space = [-1000 * ones(1, n); 
              1000 * ones(1, n)];
    ...
    for gen = 1:maxgen
        ...
        [Elite, EliteFit] = selbest(Oldpop, Fvpop, [1 1]); % vyberieme 2 najlepsie kopie najlepsieho jedinca selekcia 
        ...
        if mod(gen, 20) == 0
            fprintf('Generacia %d: Best Fitness = %.4f\n', gen, best_history(gen));
        end
