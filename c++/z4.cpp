...
void quickSort(int *data, const size_t low, const size_t high) {
    if (low + 1 < high) {
        ...
        size_t pivotIndex = getPivotIndex(data, low, high);
        pivotIndex = partition(data, pivotIndex, low, high);
...
void printWeight(Weight const *data, const size_t length) {
    for (size_t i = 0; i < length; i++) {
        std::cout << data[i].packing << " ";
        std::cout << data[i].product << "   ";
    }
    std::cout << std::endl;
}
...
int main() {
    ...
    /*int arr[] = {4,5,7,3,2,6,1};
    int size = sizeof(arr) / sizeof(arr[0]);
    printArr(arr, size);
    bubbleSort(arr, size);
    printArr(arr, size);*/ // uloha 1

    /*Weight prods[] = {{10, 1}, {20, 2}, {5,2}, {25, 10}, {1, 2}};
    size_t length = sizeof(prods) / sizeof(prods[0]);
    printWeight(prods, length);
    bubbleSort(prods, length);
    printWeight(prods, length);*/ // uloha 2

    /*int data[] = {10, 20, 30, 40, 50, 60, 70, 50, 80, 90};
    printArr(data, sizeof(data) / sizeof(int));
    std::cout << partition(data, 4, 2, 8) << std::endl;
    printArr(data, sizeof(data) / sizeof(int));*/ // uloha 4
